...
Mermaid ERD에서 선은 데이터 흐름이 아니라 테이블 간 관계를 의미합니다.
...
1차 개발에서는 삭제 처리를 단순화하기 위해 실제 삭제를 우선 사용합니다.
다만 운영 안정성을 고려해, 향후에는 `deleted` 상태값을 추가하거나 `deleted_at` 컬럼을 사용하는 soft delete 방식을 검토합니다.
